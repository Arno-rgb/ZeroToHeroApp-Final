        ...
        try {
            ...
            if (!user) {
                 ...
                 const newUser: UserData = {
                    id: userId, name: 'Hero', level: 1, tier: 0, experience: 0, energy: 100, maxEnergy: 100, ledgerRevision: 0,
            ...
            const userDataWithEnergy: UserData = {
                ...
                ledgerRevision: typeof user.ledgerRevision === 'number' ? user.ledgerRevision : initialState.ledgerRevision,
            };
            if (userDataWithEnergy.ledgerRevision !== user.ledgerRevision) {
                await db.users.update(userDataWithEnergy.id, { ledgerRevision: userDataWithEnergy.ledgerRevision });
            }
...
export const userSlice = createSlice({
  ...
  reducers: {
    ...
    setLedgerRevision: (state, action: PayloadAction<number>) => { state.ledgerRevision = action.payload; },
  },
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchUser.fulfilled, (state, action: PayloadAction<UserData>) => {
        ...
        state.ledgerRevision = typeof action.payload.ledgerRevision === 'number' ? action.payload.ledgerRevision : initialState.ledgerRevision;
