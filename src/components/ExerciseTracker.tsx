...
import { addExercise, fetchTodayExercises, Exercise, ExerciseType } from '../features/exercise/exerciseSlice'; // Adjust path if needed
...
import { displayVolume } from '../domain/training';
...
const ExercisePage: React.FC = () => {
    ...
    const handleCameraExerciseComplete = async (data: { type: ExerciseType; count: number; formQuality: number }) => {
        ...
        try {
            ...
            const savedExercise = await dispatch(addExercise(exerciseDataPayload)).unwrap();
            dispatch(addExperience(data.count));
            toast({ title: 'Saved from Camera!', description: `Credited ${displayVolume(savedExercise.type, savedExercise.creditedVolume)} toward permanent stats.`, status: 'success' });
