...
const getCharacterImage = (avatar: { costume: string; color: string } | undefined): string => {
    ...
    const costume = avatar?.costume || 'basic';
    const availableCostumes = new Set(['basic', 'novice', 'rising', 'elite', 'onepunch']);
    return `/images/avatars/${availableCostumes.has(costume) ? costume : 'basic'}.png`;
};
...
const Dashboard: React.FC = () => {
    ...
    const userLedgerRevision = useSelector((state: RootState) => state.user.ledgerRevision);
    ...
    const progressionError = useSelector((state: RootState) => state.progression.error);
    ...
    useEffect(() => {
        ...
        if (
            ...
            (progressionStatus === 'idle' || statSnapshot?.ledgerRevision !== userLedgerRevision)
        ) {
            dispatch(fetchStatSnapshot(userId));
        }
    }, [userStatus, exerciseStatus, progressionStatus, statSnapshot?.ledgerRevision, userLedgerRevision, userId, dispatch]);
    ...
    const attributes = statSnapshot?.attributes ?? {
        strength: 1,
        force: 1,
        endurance: 1,
        agility: 1,
    ...
    const lifetimeCredited = statSnapshot?.lifetimeCredited;
    const hasTitanImpact = statSnapshot?.unlockedAbilityIds.includes('titan-impact') ?? false;
    const buildIdentity = useMemo(() => {
        const entries = [
            ['Power', attributes.strength],
            ['Speed', attributes.endurance + attributes.agility],
            ['Tank', attributes.stability],
            ['Force', attributes.force],
        ] as const;
        return [...entries].sort((a, b) => b[1] - a[1])[0][0];
    }, [attributes.agility, attributes.endurance, attributes.force, attributes.stability, attributes.strength]);
    ...
    // --- Render Logic ---
    const isLoading = (userStatus === 'loading' || userStatus === 'idle') || (exerciseStatus === 'loading' && todayExercises.length === 0) || (progressionStatus === 'loading' && !statSnapshot);
    const isError = exerciseStatus === 'failed' || userStatus === 'failed' || progressionStatus === 'failed'; // Check all load errors
    const errorMsg = exerciseError || progressionError;
            ...
            >
                ...
                <GridItem area="resources" bg={sectionBg} p={4} borderRadius="xl" borderWidth="1px" borderColor={sectionBorderColor} boxShadow="md">
                     <VStack spacing={3} align="stretch">
                         <Heading size="sm" textAlign="center">PERMANENT BUILD</Heading>
                         <Text fontSize="xs" textAlign="center" color="gray.500">{buildIdentity} style</Text>
                         <SimpleGrid columns={2} spacing={2}>
                            <Box>
                                <Text fontSize="xs" color="gray.500">Strength</Text>
                                <Text fontWeight="bold">{attributes.strength}</Text>
                            </Box>
                            <Box>
                                <Text fontSize="xs" color="gray.500">Endurance</Text>
                                <Text fontWeight="bold">{attributes.endurance}</Text>
                            </Box>
                            <Box>
                                <Text fontSize="xs" color="gray.500">Stability</Text>
                                <Text fontWeight="bold">{attributes.stability}</Text>
                            </Box>
                            <Box>
                                <Text fontSize="xs" color="gray.500">Force</Text>
                                <Text fontWeight="bold">{attributes.force}</Text>
                            </Box>
                         </SimpleGrid>
                         <Tag colorScheme={hasTitanImpact ? 'yellow' : 'gray'} justifyContent="center">
                            {hasTitanImpact ? 'Titan Impact unlocked' : 'Titan Impact at 10,000 push-ups'}
                         </Tag>
                         <Text fontSize="xs" color="gray.500" textAlign="center">
                            Upper credited: {Math.round(lifetimeCredited?.upper_strength || 0)}
                         </Text>
                     </VStack>
                </GridItem>
